...
public class Sovrin {
    ...
    public static String anonCrypt(String walletName, String did, String payload) throws Exception {
        if (! LibIndy.isInitialized()) LibIndy.init(new File("./lib/libindy.so"));
        Pool pool = Pool.openPoolLedger(NETWORK_NAME, "{}").get();
        Wallet wallet = Wallet.openWallet(walletName, null, null).get();
        String key = Did.keyForDid(pool, wallet, did).get();
        byte[] encrypted = Crypto.anonCrypt(key, payload.getBytes()).get();
    ...
    public static String anonDecrypt(String walletName, String did, String payload) throws Exception {
        if (! LibIndy.isInitialized()) LibIndy.init(new File("./lib/libindy.so"));
        Pool pool = Pool.openPoolLedger(NETWORK_NAME, "{}").get();
        Wallet wallet = Wallet.openWallet(walletName, null, null).get();
        String key = Did.keyForDid(pool, wallet, did).get();
        byte[] decoded = Base64.getDecoder().decode(payload);
        byte[] decrypted = Crypto.anonDecrypt(wallet,key, decoded).get();
        wallet.closeWallet().get();
        pool.closePoolLedger().get();
